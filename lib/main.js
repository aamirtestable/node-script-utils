...
let _browser;
if(typeof browser !== 'undefined' && _.isUndefined(browser.testableLogInfo)){
...
const registerWdioCommands = function(brwsr) {
	console.log('--------------------------------- INSIDE TESTABLE UTILS');
	console.log(typeof brwsr);
	
	if(brwsr && typeof brwsr !== 'undefined' && _.isUndefined(browser.testableLogInfo)){
	  registerCommands = true;
	  _browser = brwsr;
	}
    
    if (registerCommands) {
        const browsers = [ _browser ];
